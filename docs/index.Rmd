...
source("docs/nsw_eff_R_data_and_plot_prep.R")
```
...
vals <- covid %>%
          ...
          arrange(date) %>%
          top_n(21, wt=row_number()) %>%
          pull(cases)

maxpos <- which(vals == max(vals))
if (all(vals[maxpos] > vals[18:21])) {
  fig(datafy(vals, "covid19visualisations"), size=80,
      symbol=setNames(c(maxpos, 18:20), c("max", rep("down",3))))
} else {
  fig(datafy(vals, "covid19visualisations"), size=80,
      symbol=setNames(c(maxpos, 18:20), c("max", rep("up",3))))
}
