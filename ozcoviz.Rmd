...
```{r effective-one, fig.height = 8, fig.width = 12}

covid_instant_r_diff_1 <- covid_instant_r %>% 
  arrange(country_region, date)  %>% 
  group_by(country_region) %>% 
  mutate(one_diff = 1 - median_r)  %>% 
  ungroup()

  ggplot(covid_instant_r_diff_1,
         aes(x = date,
             y = one_diff,
  ...
  labs(title = "Distance from Effective R of 1",
       subtitle = "Above 0 is better, below zero is worse")
```

***

We can take these graphics and look at how different they are from 1, which indicates that an outbreak is under control.
