...
* an upper group of just one, the USA, where the epidemic has grown rapidly until reaching a plateau around 35 days since commencement.
...
Canada clearly suffers from sharing a long border with the US, but nonetheless appears to almost have its COVID-19 epidemic under control, or at least, close to a decay phase.
...
The world is aghast at the US reponse to COVID-19 and the results thereof. But they do appear to be slowly reducing transmission, mainly due to extremely rigorous social distancing measures in New York city and state, which is a centre of the epidemic.
...
```{r flatten-this, eval=FALSE}
p <- tibble(age=rep(1:100,100),
       step=rep(1:100, each=100),
       coeff=rep(seq(1.0, 0.05,length.out=100),each=100)) %>%
  mutate(flout = dlnorm(age,3.3,0.35)*coeff) %>%
  ggplot(aes(x=age, y=flout, fill=step)) +
  geom_area() +
  xlim(0,90) +
  scale_fill_gradient(low = "red", high = "green", na.value = NA) +
  labs(x="Age in years",
       y="Number of people ignoring\nsocial distancing rules",
       title="Flatten this!",
       caption="Source: observational data from
                Bondi Junction & Centennial Park,
                2-6 pm, Sat 18 April 2020.
                Tim Churches, Sydney, Australia") +
  theme_minimal() +
  easy_remove_y_axis(what = c("text")) +
  easy_remove_legend() +
  easy_all_text_size(size = 16) +
  easy_y_axis_title_size(size=22) +
  easy_x_axis_title_size(size=22) +
  easy_plot_title_size(size = 30) +
  easy_plot_caption_size(size=14) +
  theme(plot.title = element_text(hjust = 0.5)) +
  transition_time(step)

animate(plot=p, width=600, height=600, start_pause=5, end_pause=5, rewind=TRUE)
```

```{r flatten-this-gif}
if (params$local) {
  knitr::include_graphics("docs/assets/flatten-this.gif")
} else {
  knitr::include_graphics("assets/flatten-this.gif")
}
```

***

A tongue-in-cheek infographic motivated by the observation that, as at 18th April 2020,  many people in particular demographics appear to have assumed that the whole COVID-19 thing is over in Australia, and that it's party time...

### Bhatia/Minute Physics chart

```{r cum-incidence-incidence-fig2, fig.height = 8, fig.width = 12}
if (params$local) {
  knitr::include_graphics("docs/assets/log-log-incidence.gif")
} else {
  knitr::include_graphics("assets/log-log-incidence.gif")
